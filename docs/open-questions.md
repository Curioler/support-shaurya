# Open questions

Things that must be verified with Gaurang or the hospitals before they appear in any
public-facing campaign material. Anything unresolved here is a credibility risk.

**Last updated:** 2026-08-15

---

## Urgent — these gate everything else

| # | Question | Why it matters | Status |
|---|---|---|---|
| 16 | **Did deflazacort start around 6 July 2026, as Dr Chalipat planned?** | Still the most time-sensitive item. Prescribed 25 May as 12 mg once daily, to begin after 6 weeks. If it started on schedule he clears 12 weeks ≈ 28 Sep 2026 and 24 weeks ≈ 21 Dec 2026 — the SGT-003 and FORZETTO gates. If it slipped, both move. One question to Gaurang answers it. See [the steroid clock](outreach/steroid-clock-and-eligibility.md). | Open |
| 12 | **Has Shaurya been tested for anti-AAVrh74 antibodies, and what was the result?** | ELEVIDYS has now been **formally prescribed**, and it cannot be given without a negative result. Seven clinical documents and six clinicians in, no document records this test. If positive, the ₹26 crore purchase is not possible no matter how much is raised. It is an ordinary blood test. | Open |
| 5 | Is there a treatment deadline or clinical window? | **Answered by the treating physician**, in writing: ELEVIDYS "works in a time limited fashion and works better if given earlier". Licensed from age 4; Shaurya is 4y7m and ambulatory. The urgency is now attested, not asserted — the appeal should say so. | **Closed** |
| 29 | **What is the full landed cost, including taxes and hospital charges?** | The certificate states ₹26 crore is the drug **excluding taxes**, and says nothing about immunosuppression, admission or post-infusion monitoring. India exempts rare-disease personal imports from basic customs duty, but that must be applied for and does not cover every levy. Hitting ₹26 crore and then finding the therapy cannot be released for want of another sum is foreseeable from this sentence. | Open — **material** |
| 34 | **Are the two campaigns coordinated, and who decides messaging across them?** | Father runs ImpactGuru, mother runs Ketto, with different goals and different copy. Without one person owning the message, the two pages will keep drifting apart. Relates to [#7](#people-and-governance). | Open |

## Medical — clinical status

| # | Question | Why it matters | Status |
|---|---|---|---|
| 22 | What happened between 21 May and 15 August 2026? | Three months with no documentation, covering exactly the period when steroids, cardiac workup and AAV testing should have happened. See [medical-timeline.md](medical-timeline.md). | Open |
| 17 | Were the ECG and 2D Echo done with Dr Chintan Bhatt, and what did they show? | Ordered 21 May 2026 to look for diastolic dysfunction. Cardiac function is an exclusion criterion in several trials. | Open |
| 19 | Was the vaccination card produced? | It was the blocker on starting steroids, which is the blocker on trial eligibility. | Open |
| 21 | Can NSAA, rise-from-floor time and 10 m walk/run be recorded at the next clinic visit? | Ordinary measurements his clinic already knows how to take, and trial sites ask for them by name. PBGENE-DMD needs NSAA 16–29; FORZETTO needs rise-from-floor <10 s. Their absence weakens every enquiry. | Open |
| 13 | Is Shaurya on a stable corticosteroid regimen, and for how long? | Most trials require 12–24 weeks of stable steroids before screening. | Open — see #16 |

## Genetics

| # | Question | Why it matters | Status |
|---|---|---|---|
| 11 | What is Shaurya's exact DMD mutation? | Hemizygous out-of-frame deletion of **exon 50**, dosage quotient 0.00, pathogenic, no VUS, no other copy-number change. [MedGenome, 19 May 2026](sources/2026-05-19-medgenome-dmd-mlpa-report.md). Amenable to exon 51 skipping. | **Closed** |
| 18 | Can we obtain the MedGenome MLPA report itself? | Received 2026-08-15, filed in sources. | **Closed** |
| 23 | **Has exon 50 sequencing been done?** | MedGenome explicitly recommends it, and their own disclaimer says single-exon MLPA deletions "should always be confirmed by other methods like multiplex PCR or sequencing". Sponsors review genotype through their own genetic counsellors; an unconfirmed single-exon call is what stalls an application. | Open |
| 24 | Has Shaurya's mother been screened for carrier status? | Recommended by MedGenome and **ordered by Dr Chalipat on 25 May 2026** (CPK, DMD-MLPA, 2D Echo). Whether it was done is unknown. | Open — ordered |
| 25 | What is Shaurya's exact date of birth? | **CLOSED — 7 January 2022**, per Ankura Hospital records. He is 4y7m, not the 4y4m in the campaign material. Givinostat window closes 7 Jan 2028. | **Closed** |
| 27 | Is the campaign's stated age being kept current? | The appeal says 4 years 4 months; that was true in May and is now wrong. A stale age in a live fundraising appeal is a small credibility leak that compounds as months pass. | Open |

## Treatment strategy

| # | Question | Why it matters | Status |
|---|---|---|---|
| 10 | Is a **clinical trial** being pursued as well as **paid ELEVIDYS**? | Two different strategies with different timelines and costs. The campaign describes only the paid route; a trial place would change the ask entirely. | Open |
| 4 | Where would ELEVIDYS be administered? | **Partly answered — SPARSH Hospital, Bengaluru**, prescribed 9 July 2026 by Dr Ann Agnes Mathew. 140 mL / 14 vials, single infusion under 120 minutes. | **Partly closed** |
| 28 | **Under what regulatory route is ELEVIDYS being imported?** | It is US-approved, not an Indian-approved product. Administration in Bengaluru would ordinarily need a named-patient / compassionate import authorisation. The prescription does not say which route, how long it takes, or whether it has been applied for. For a ₹26 crore campaign, the pathway matters as much as the money — funds raised against an unavailable import route help nobody. | Open |
| 3 | Is ₹26 crore the therapy cost alone, or does it include hospitalisation, workup, immunosuppression, and post-infusion monitoring? | Determines whether the target is the real target. Now costable against a concrete quantity: **14 vials**. Under-asking is worse than over-asking. | Open |
| 15 | Would the family travel internationally for a trial, and is that funded? | No DMD gene therapy or exon-skipping trial is recruiting in India. Every live route means travel, visas, accommodation and a carer — costs the campaign has not accounted for. | Open |
| 9 | Which trial sites has the family already contacted, when, and what came back? | Several addresses in the original outreach list were undeliverable, so silence may mean the mail never arrived rather than a "no". Without a contact log, sites get re-mailed or missed. | Open |

## People and governance

| # | Question | Why it matters | Status |
|---|---|---|---|
| 20 | Who is the lead clinician overall? | **Likely Dr Ann Agnes Mathew** at SPARSH Bengaluru — she holds a Fellowship in Paediatric Neuromuscular Diseases and wrote the ELEVIDYS prescription, which is the most consequential decision in the file. Care still spans seven names across four cities (Dr Joshi/Vadodara, Dr Chalipat/Pune, Dr Chintan Bhatt, Dr Sarbani Raha, Dr Biren Bhatt, Dr Mathew/Bengaluru). Should be confirmed rather than inferred — trial sites ask who coordinates. | Open |
| 26 | How do Dr Sarbani Raha and Dr Biren Bhatt relate? | Two different referring clinicians on samples drawn a day apart, both via Toprani Labs. Not a problem, but the campaign should be able to say who ordered what. | Open |
| 7 | Who is authorised to speak publicly for the campaign and approve material? | Prevents contradictory messaging across volunteers. | Open |
| 8 | Which specific medical details has the family consented to making public? | Minor's medical data. Consent scope must be explicit, not assumed. | Open |

## Campaign mechanics

| # | Question | Why it matters | Status |
|---|---|---|---|
| 6 | How much has been raised so far? | **Answered 2026-08-15** — ImpactGuru ₹6,12,343 (418 donors) + Ketto ₹5,38,978 (216 supporters) = **₹11,51,321**, about 0.44% of ₹26 crore. Nobody is publishing the combined figure. See [fundraising-channels.md](campaign/fundraising-channels.md). | **Answered — needs a live tracker** |
| 31 | **Which goal figure is correct — ₹20 crore or ₹26 crore?** | ImpactGuru asks ₹26 crore, Ketto asks ₹20 crore, and the physician's certificate says ₹26 crore *excluding taxes* — so ₹20 crore is below the drug cost alone. A donor opening both links sees two different asks for the same treatment. | Open — **fix in public copy** |
| 32 | Ketto appeal body says "My name is **Shaurya Nayak**, and I am fighting to save my son's life" | Shaurya is the patient; the campaigner is his mother. Live public copy, second paragraph, on a page written in the mother's voice throughout. Almost certainly a template slip. | Open — **fix in public copy** |
| 33 | ImpactGuru beneficiary panel reads "Shaurya Nayak — **Parent of** Gaurang Nayak" | Inverted relationship, sitting inside the section carrying the *Verified* badges. Support can correct it. | Open — **fix in public copy** |
| 1 | Is the UPI VPA exactly `supportshaury9484@cashfreensdlpb`? | **Downgraded.** ImpactGuru states UPI donations route to Impact Guru's own account, not a personal one, and Cashfree is its gateway — so the "shaury" spelling is a generated handle, not a typo. A ₹1 test is still worth doing, but this is no longer a serious risk. | Low priority |
| 2 | "spent ₹1 lakh on his medical care" — is this the intended figure? | It sits oddly next to "exhausted our savings". If it's a typo for ₹10 lakh or more, the appeal reads as careless. | Open |
| 30 | Should public copy describe ELEVIDYS as a "cure"? | The physician's certificate uses the word; the family's own appeal correctly says the therapy "offers hope for slowing the disease". ELEVIDYS is not curative. Donors and journalists check high-profile therapies, and a correction on this point would undermine everything else the campaign says. Use the certificate unaltered where an attestation is needed; use the appeal's wording in public copy. | Open — **decide before scaling** |
